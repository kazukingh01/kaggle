...
tokenizer.itos = {y:x for x, y in tokenizer.stoi.items()}
...
if __name__ == "__main__":
    ...
    if args.get("data") is None:
        norm_mean, norm_std = calc_normalize_mean_std(df_train["filepath"].values.tolist(), IMAGE_IN_SIZE, samples=10000, n_jobs=40)
    ...
    dataset_valid = create_dataset(df_valid.copy(), norm_mean[0], norm_std[0])
    # DataLoader
    ...
    dataloader_valid = torch.utils.data.DataLoader(
        dataset_valid, batch_size=32, shuffle=False, num_workers=0, 
        drop_last=False, collate_fn=partial(collate_fn, valid=True)
    )
    ...
    if args.get("pred") is not None:
        ndf, _ = trainer.predict_proba_dataloader(dataloader=dataloader_valid)
        ndf = np.argmax(ndf[0], axis=2)
        df_pred = pd.DataFrame(tokenizer.sequences_to_texts(ndf), columns=["text"])
        df_pred["index"] = df_pred["text"].str.find("<eos>")
        df_pred["pred"]  = df_pred.apply(lambda x: x["text"][:x["index"]], axis=1)
        df_valid["pred"] = df_pred["pred"].values